...
def extractAirData(rawData):
    ...
    airData['T_C'] = ((rawData[0] & TEMPERATURE_VALUE_MASK) +
    ...
    airData['P_Pa'] = ((rawData[5] << 24) + (rawData[4] <<
...
def extractAirQualityData(rawData):
    ...
    airQualityData['AQI'] = rawData[0] + \
        (rawData[1] << 8) + (float(rawData[2])/10.0)
    airQualityData['CO2e'] = rawData[3] + \
        (rawData[4] << 8) + (float(rawData[5])/10.0)
    airQualityData['bVOC'] = rawData[6] + \
        (rawData[7] << 8) + (float(rawData[8])/100.0)
...
def extractLightData(rawData):
    ...
    lightData['illum_lux'] = rawData[0] + \
        (rawData[1] << 8) + (float(rawData[2])/100.0)
...
def extractSoundData(rawData):
    ...
    soundData = {'SPL_dBA': 0, 'SPL_bands_dB': [
        0]*SOUND_FREQ_BANDS, 'peak_amp_mPa': 0, 'stable': 0}
    soundData['SPL_dBA'] = rawData[0] + (float(rawData[1])/10.0)
    j = 2
    for i in range(0, SOUND_FREQ_BANDS):
        soundData['SPL_bands_dB'][i] = rawData[j] + \
            (float(rawData[j+SOUND_FREQ_BANDS])/10.0)
        j += 1
    j += SOUND_FREQ_BANDS
    soundData['peak_amp_mPa'] = rawData[j] + \
        (rawData[j+1] << 8) + (float(rawData[j+2])/100.0)
...
def extractParticleData(rawData, particleSensor):
    ...
    particleData = {'duty_cycle_pc': 0, 'concentration': 0,
                    'conc_unit': "", 'valid': False}
    particleData['duty_cycle_pc'] = rawData[0] + (float(rawData[1])/100.0)
    particleData['concentration'] = rawData[2] + \
        (rawData[3] << 8) + (float(rawData[4])/100.0)
    if (rawData[5] > 0):
...
def writeAirData(textFileObject, airData, writeAsColumns):
    ...
    if (writeAsColumns):
        ...
        textFileObject.write("Gas Sensor Resistance = " +
                             str(airData['G_ohm']) + " ohm\n")
...
def writeLightData(textFileObject, lightData, writeAsColumns):
    ...
    if (writeAsColumns):
        ...
        textFileObject.write("White Light Level = " +
                             str(lightData['white']) + "\n")
...
def writeSoundData(textFileObject, soundData, writeAsColumns):
    ...
    if (writeAsColumns):
        ...
        for i in range(0, SOUND_FREQ_BANDS):
            textFileObject.write("{:.1f} ".format(
                soundData['SPL_bands_dB'][i]))
...
def writeParticleData(textFileObject, particleData, writeAsColumns):
    ...
    if (writeAsColumns):
        ...
        textFileObject.write("Particle Sensor Duty Cycle = {:.2f} %\n".format(
            particleData['duty_cycle_pc']))
        textFileObject.write("Particle Concentration = {:.2f} ".format(
...
def startNewDataFile(dataFileDirectory):
    filename = os.path.join(dataFileDirectory, datetime.datetime.now().strftime(
...
def setSoundInterruptThreshold(I2C_bus, sound_thres_mPa):
    ...
    I2C_bus.write_i2c_block_data(
        i2c_7bit_address, SOUND_INTERRUPT_THRESHOLD_REG, data_to_send)
...
def setLightInterruptThreshold(I2C_bus, light_thres_lux_i, light_thres_lux_f2dp):
    ...
    I2C_bus.write_i2c_block_data(
        i2c_7bit_address, LIGHT_INTERRUPT_THRESHOLD_REG, data_to_send)
